var graph = d3.select("#party").selectAll("div");
...
var dem = function(){
    var graph = d3.select("#party").selectAll("div");
    graph.transition(750).attr("class","dem").style("width",function(d){return dscale(ddel[d])+"%";}).text(function(d){return d+" "+ddel[d];});
}

var rep = function(){
    var graph = d3.select("#party").selectAll("div");
    graph.transition(750).attr("class","rep").style("width",function(d){return rscale(rdel[d])+"%";}).text(function(d){return d+" "+rdel[d];});
}

graph.data(states).enter().append("div").attr("class","dem").style("width",function(d){return dscale(ddel[d])+"%";}).text(function(d){return d+" "+ddel[d];});
